...
%     newWidth = (int32)c/n;
    ...
    sizetwo = double(int32(k*size));
    sizetwo = mod(sizetwo+1,2)+sizetwo;
    
    filterOne = fspecial('gaussian',size,sigma);
    filterTwo = fspecial('gaussian',sizetwo,k*sigma);
    filteredImage = imfilter(greyImage,filterTwo,'same')-imfilter(greyImage,filterOne,'same');
    results(:,:,n) = abs(filteredImage)/(k-1) ;
...
title(sprintf('DOG. Threshold: %0.4f',threshold));
